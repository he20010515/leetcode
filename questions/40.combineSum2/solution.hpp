 ...
 * @LastEditTime: 2024-03-17 13:50:53
...
public:
    ...
    bool dfs(vector<int> &candidates, int target, vector<int> &buffer, vector<vector<int>> &res, int forbegin)
    {
        ...
        {
            if (k > forbegin and candidates[k - 1] == candidates[k]) // 每组数中, 一串相同的数中的第一个肯定要考虑, 其他的continue出去.
                continue;
            buffer.push_back(candidates[k]);
            dfs(candidates, target, buffer, res, k + 1); // 第k个数已经用了, 所以从k+1个数继续考虑
            buffer.pop_back();
        }
        return false;
    };
